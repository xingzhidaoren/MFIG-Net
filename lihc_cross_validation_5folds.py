...
if __name__ == '__main__':
    ...
    frm_gama = 0.3#the ratio to masking

    categories = ['bad response', 'good response']
    ...
    for fold, (traind_ids, val_ids) in enumerate(kfold.split(dataset)):
        ...
        trainer = ClassifierTrainer(model, optimizer='adamw', batch_size=128, epochs=100, 
                                    lr=0.0001, lr_decay_steps=100, use_focal_loss=True, device=device)
        
        ckpoint_save_path = 'checkpoints/'+ build_fold_path('lihc', classifier, frm_gama, fold+1)
        ...
        for mask_r in [0.3]:
            metric_save_path = 'results/' + build_fold_path('lihc', classifier, frm_gama, fold+1) + '/maskr{0}'.format(mask_r)
